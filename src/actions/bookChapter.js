...
export const getChapter = (options, data)=>{
    return dispatch => {
        //必须list设置空 才能触发shouldComponentUpdate
        dispatch(handle({
            loading: true
        }))
        ...
        storage.load({
            ...
            console.log(err)
            /////未加入书架, 废弃的内容
            //setDom(data, data.pageList, data.sourceType)
        });
        ...
        function setDom(ret, pageList, oldSourceType){
            ...
            let sort = !!options.sort? options.sort: ret.sort;
            console.log('判断'+ options.sourceType)
            ////有数据,且不传来源,且不是更新章节列表,  代表只排序
            if(pageList.length>0 && !!!options.sourceType && !!!options.isupdate){
                ///////可以设置排序方式
                ...
                Fetch({
                    url: ur,
                    type:"GET",
                    data: {id: options.id, name: ret.name, author: ret.author, authorId:  ret.authorId, sourceType: sourceType},
                    success:function(result){
                        if(result.status==1){
                            ...
                            //判断长度,来源也不传，则不做任何修改
                            if(ret.pageList.length==dds.pageList.length && !!!options.sourceType){
                                ...
                                if(sourceType == "qidian") dd.qidian_ptotal = dds.pageList.length;
